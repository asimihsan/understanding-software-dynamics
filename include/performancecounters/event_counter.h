...
struct event_count {
    ...
    long long cycles() const { return event_counts[CPU_CYCLES]; }

    long long instructions() const { return event_counts[INSTRUCTIONS]; }

    long long branch_misses() const { return event_counts[BRANCH_MISSES]; }

    long long branches() const { return event_counts[BRANCH]; }
